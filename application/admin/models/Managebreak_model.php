...
class managebreak_model extends CI_Model{
	...
	function getEmpBreakPunches($emp,$date){
		$sql ="SELECT 
					*
				FROM 
					`break_punches` 
	...
	function getDetailsOfBreakPun($id){
	...
	function del_breakpun($id){
		$this->db->where('bp_id',$id);
		$q = $this->db->delete($this->tbl_break);
		return $q;
	}
